...
public class UnweightedGraph<V> implements Graph<V> {
	...
	public SearchTree dfsNonRecurs(int v) {
		...
		// Add v to the stack, searchOrder and set as visited
		stack.push(v);
		searchOrder.add(v);
		isVisited[v] = true;
		...
		while (stack.isEmpty() == false) {
			...
			System.out.println("x size: " + neighbors.get(x).size());
			if (neighbors.get(x).size() == 0) {
				...
				for (int i = 0; i < neighbors.get(x).size(); i++) {
					Edge e = neighbors.get(x).get(neighbors.get(x).size() - 1);
					System.out.println("removing " + neighbors.get(x).get(i).toString() + " from x.");
